...
def Faster_HarrisDetector(src, gaus_filter_size=3, gaus_sigma=1, alpha=0.04, threshold_rate=0.01):
    ...
    # 0 ~ 1 사이의 값으로 변경 후 0 ~ 255로 변경 -> 결과가 잘 나왔는지 확인하기 위해서
...
def Sliding_Window_HarrisDetector(src, gaus_filter_size=3, gaus_sigma=1, window_size=3, alpha=0.04,
    ...
    for row in range(h):
        for col in range(w):
            start_y = row - window_size // 2
            end_y = row + window_size // 2 + 1
            start_x = col - window_size // 2
            end_x = col + window_size // 2 + 1

            Sxx = pad_G_IxIx[start_y:end_y, start_x:end_x].sum()
            Syy = pad_G_IyIy[start_y:end_y, start_x:end_x].sum()
            Sxy = pad_G_IxIy[start_y:end_y, start_x:end_x].sum()
