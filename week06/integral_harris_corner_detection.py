...
def get_integral_image(src):
    ...
    for row in range(1, h):
        for col in range(1, w):
            dst[row, col] = dst[row - 1, col] + dst[row, col - 1] - dst[row - 1, col - 1] + src[row, col]
...
def calc_local_integral_value(src, left_top, right_bottom):
    ...
    (left_top_row, left_top_col) = left_top
    (right_bottom_row, right_bottom_col) = right_bottom

    # src = integral image
    left_top_value = src[left_top_row - 1, left_top_col - 1]
    right_top_value = src[left_top_row - 1, right_bottom_col]
    left_bottom_value = src[right_bottom_row, left_top_col - 1]
    right_bottom_value = src[right_bottom_row, right_bottom_col]

    if left_top_row == 0:
        left_top_value = right_top_value = 0
    if left_top_col == 0:
        left_top_value = left_bottom_value = 0

    result = right_bottom_value - right_top_value - left_bottom_value + left_top_value
...
def harris_detector(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            ...
            det_M = M_harris[row, col, 0, 0] * M_harris[row, col, 1, 1] \
                    - M_harris[row, col, 0, 1] * M_harris[row, col, 1, 0]
...
def calc_M_integral(IxIx_integral, IxIy_integral, IyIy_integral, fsize = 5):
    ...
    # 실습 때 알려드린 integral의 지역 해당 영역 값을 구하는 함수를 완성하여 사용하면 쉽게 구할 수 있음
    ##########################################################################

    for row in range(h):
        for col in range(w):
            M[row, col, 0, 0] = calc_local_integral_value(IxIx_integral_pad, (row, col),
                                (row + fsize - 1, col + fsize - 1))
            M[row, col, 0, 1] = calc_local_integral_value(IxIy_integral_pad, (row, col),
                                (row + fsize - 1, col + fsize - 1))
            M[row, col, 1, 0] = M[row, col, 0, 1]
            M[row, col, 1, 1] = calc_local_integral_value(IyIy_integral_pad, (row, col),
                                (row + fsize - 1, col + fsize - 1))
...
def harris_detector_integral(src, k = 0.04, threshold_rate = 0.01, fsize=5):
    ...
    for row in range(h):
        for col in range(w):
            ...
            # R 계산 Harris 방정식 구현
            ##########################################################################
            det_M = M_integral[row, col, 0, 0] * M_integral[row, col, 1, 1] \
                    - M_integral[row, col, 0, 1] * M_integral[row, col, 1, 0]
...
def main():
    ...
    cv2.imshow('harris_img ' + '202002481', harris_img)
    cv2.imshow('harris_integral_img ' + '202002481', harris_integral_img)
    cv2.waitKey()
