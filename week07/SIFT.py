...
def SIFT(src):
    ...
    # cv2 내장함수를 통해서 keypoint를 추출합니다.
    print("get keypoint")
    dst = cv2.cornerHarris(gray, 3, 3, 0.04)
    dst[dst < 0.01 * dst.max()] = 0

    # keypoint를 find_local_maxima를 통해 한번 정제화
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            for col in range(-8, 8):
                ...
                ###################################################################
                ## ToDo
                ## gaussian weight와 orientation histogram 구현
                ## gaussian weight는 실습 ppt를 이용하여 구현 가능
                ## orient_hist는 10도별로 범위에 해당하는 인덱스에 magnitude와 gaussian_weight를 곱한 값을 누적시킴
                ###################################################################
                gaussian_weight = np.exp(-(row ** 2 + col ** 2) / 16)
                angle_index = angle[p_y, p_x] // 10
                orient_hist[int(angle_index)] += magnitude[p_y, p_x] * gaussian_weight
        ...
        # orient_hist에서 가중치가 가장 큰 값을 추출하여 keypoint의 angle로 설정
        keypoints[i].angle = (np.argmax(orient_hist).astype(np.float32)) * 10

        # 가장 큰 가중치의 0.8배보다 큰 값도 keypoint의 angle로 설정
        for j in range(36):
            if j != np.argmax(orient_hist):
                if orient_hist[j] > orient_hist[np.argmax(orient_hist)] * 0.8:
                    # KeyPoint(x, y, size, key_angle, response, octave, class_id)
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            for col in range(-8, 8):
                ...
                ###################################################################
                gaussian_weight = np.exp(-(row_rot ** 2 + col_rot ** 2) / 16)  # 위에서 활용한 식을 이용
                weight = magnitude[p_y, p_x] * gaussian_weight  # 위에서 활용한 식을 이용

                # 조정된 angle 값에서 keypoint의 original angle을 빼줌
                ...
                # 128 dimension에서 값을 넣을 index 계산
                index = (patch_row * 4 + patch_col) * 8 + int(descriptor_angle // 45)
...
def main():
    ...
    # 결과의 학번 작성하기!
    cv2.imshow('202002481_my_sift', result)
    cv2.waitKey()
